...
var Simplite = function() {
     ...
     * 获取dom元素
     * 
     * @private param {string|dom|jquery} idOrNode
     *          支持domId，dom元素和jquery包装的dom元素方式
     ...
     * 获取指定dom元素的字符串模板
     * 
     * @private param {string|dom|jquery} idOrNodeOrHtml
     *          支持domId，dom元素和jquery包装的dom元素方式或者html字符串
    ...
    var parse = function(text) {
        var out = 'var out = "";';
        var segments = parseOpenTag(text, Simplite.logicOpenTag);
        for (var i = 0, len = segments.length; i < len; i++) {
            var segment = segments[i];
            var jsAndHtml = parseCloseTag(segment, Simplite.logicCloseTag);
            var js = jsAndHtml[0];
            ...
            if (html) {
                if (Simplite.attrOpenTag !== Simplite.logicOpenTag
                    ...
                    var htmlSegments = parseOpenTag(html, Simplite.attrOpenTag);
                    for (var j = 0, l = htmlSegments.length; j < l; j++) {
                        var htmlSegment = htmlSegments[j];
                        var attrAndHtml = parseCloseTag(htmlSegment, Simplite.attrCloseTag);
                        var attr = attrAndHtml[0];
                        var html = attrAndHtml[1];
                        if (html === undefined) {
                            html = attr;
                            attr = '';
                        }
     ...
     * 
     * @private
     * @param {string|Dom|Jquery}
     *            template 模板元素或者模板html
     * @param {object}
     *            data 用来填充模板的数据
     ...
     *            options 初始化配置参数
     * @param {string|Dom|Jquery}
     *            option.target 模板填充到的指定元素，支持domId，dom元素和jquery包装的dom元素方式
     * @param {string|Dom|Jquery}
     *            option.template 模板元素或模板内容
     */
